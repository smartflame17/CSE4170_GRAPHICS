...
enum STATIC_OBJECT_ID {
	...
	// added from data file
	STATIC_OBJECT_BIKE, STATIC_OBJECT_CAT, STATIC_OBJECT_TANK, STATIC_OBJECT_HELICOPTER, STATIC_OBJECT_GODZILLA
};

enum DYNAMIC_OBJECT_ID {
	DYNAMIC_OBJECT_TIGER = 0, DYNAMIC_OBJECT_COW_1, DYNAMIC_OBJECT_COW_2, DYNAMIC_OBJECT_NATHAN, DYNAMIC_OBJECT_WOLF
};

enum SHADER_ID { SHADER_SIMPLE = 0, SHADER_PHONG, SHADER_GOURAUD, SHADER_PHONG_TEXUTRE };

struct Light_Properties {
	glm::vec4 position; // In EC
	glm::vec4 ambient_color, diffuse_color, specular_color;
	glm::vec4 light_attenuation_factors;
	glm::vec3 spot_direction; // In EC
	float spot_exponent;
	float spot_cutoff_angle;
	bool light_on;
};

struct loc_Light_Properties {
	GLint position;
	GLint ambient_color, diffuse_color, specular_color;
	GLint light_attenuation_factors;
	GLint spot_direction;
	GLfloat spot_exponent;	//GLint originally
	GLfloat spot_cutoff_angle;  //GLint originally
	GLint light_on;
};
...
// new struct for holding material info as GLint
struct loc_Material {
	GLint emission, ambient, diffuse, specular;
	GLfloat exponent;
};

// new function for converting Material's info to loc_Material
...
struct Shader_Phong : Shader {
	...
	GLint loc_n_lights;
	loc_Light_Properties loc_lights[N_MAX_LIGHTS];
...
struct Axis_Object {
	...
	void define_axis();
	void draw_axis(Shader_Simple* shader_simple, glm::mat4& ViewMatrix, glm::mat4& ProjectionMatrix, glm::mat4& ModelMatrix);
};
...
// added from data file
struct Nathan_D : public Dynamic_Object {
	Nathan_D(DYNAMIC_OBJECT_ID _object_id) : Dynamic_Object(_object_id) {}
	void define_object();
};
...
struct Dynamic_Geometry_Data {
	...
	Nathan_D nathan_d{ DYNAMIC_OBJECT_NATHAN };
...
struct Scene {
	...
	Camera_ID current_active_cc_cam_id;	// Stores which camera on mini viewport is active
	...
	// Light related
	Light_Properties lights[N_MAX_LIGHTS];
	int n_lights;
	...
	Scene() {
		...
		shader_list.clear();
		camera_list.clear();
		shader_kind = SHADER_GOURAUD;
		ViewMatrix = ProjectionMatrix = glm::mat4(1.0f);
		current_active_cc_cam_id = CAMERA_CC_0;		// default mini cam view is cam0
		draw_axis_flag = false;						// don't draw axis by default
		n_lights = 0;
	}
	...
	void set_active_cc_camera(Camera_ID id);
